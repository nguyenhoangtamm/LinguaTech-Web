...
import {
    ...
    CreateRoleBodyType,
    UpdateRoleBodyType,
    ConfigMenuWithRoleResType, ActionConfigMenuWithRoleType,
...
import { AssignRoleToUserResType, AssignRoleToUserType } from "@/schemaValidations/user.schema";
...
const roleApiRequest = {
    ...
    list: (filters: {
        ...
        pageSize: number,
        keyword?: string
    }) => http.get<RoleListResType>(buildUrlWithParams('/roles/get-pagination', filters)),
    getRole: (id: number) => http.get<RoleResType>(`roles/get-by-id/${id}`),
    create: (body: CreateRoleBodyType) => http.post<RoleResType>('/roles/create', body),
    ...
    deleteRole: (id: number) => http.post<RoleResType>(`roles/delete/${id}`, null),
    getConfigMenuIds: (id: number) => http.get<ConfigMenuWithRoleResType>(`/roles/get-config-menu/${id}`),
    ...
    getConfigPermissionsByRoleId: (id: number) =>
        http.get<RolePermissionsByRoleIdResType>(`/roles/get-permissions-by-role/${id}`),
