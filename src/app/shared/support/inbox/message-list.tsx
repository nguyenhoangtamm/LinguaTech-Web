...
import { LineGroup, Skeleton } from "@/components/ui/skeleton-inbox";
...
export const messageIdAtom = atomWithStorage<string | null>("messageId", null);
...
export function MessageItem({ className, message }: MessageItemProps) {
  ...
  function handleChange() {
    ...
    if (isMobile) {
      const url = routes.manage.support.messageDetails(message.id);
      router.push(url);
    }
  }
  ...
  return (
    <>
      <div className={cn(className, "sticky")}>
        ...
        <div className="overflow-hidden rounded-lg border border-muted pt-3">
